...
<div class="wrapper container">
  ...
  <div class="row">
    ...
    <section class="col-md-24">
      ...
      <p>Выберите период, за который вы хотите посмотреть изминения курса.</p>
      <form:form method="get" action="select1">
              <spring:message code="label.from" />
            <input type="date" path="from" />
              <spring:message code="label.to" />
            <input type="date" path="to" />
            <input type="submit" value="<spring:message code="label.getRates"/>" />
      </form:form>

      <div class = "text-success">
        <c:if test="${!empty selectRates1}">
          <table id="currency-table" class="table table-hover table-bordered" >
            ...
            <tbody>
            <c:forEach items="${selectRates1}" var="rate">
            <tr>
              <td>${rate.TTime}</td>
              <td>${rate.USDbuy}</td>
              <td>${rate.USDsell}</td>
